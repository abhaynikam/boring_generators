  ...
  module Favicon
    module Build
      class BuildGenerator < Rails::Generators::Base
        ...
        class_option :template_image_path,  type: :string, aliases: "-tp",
                                            desc: "Tell us the favicon template image file path. Default to primary: #{DEFAULT_IMAGE_PATH}"
        ...
        def build_favicon_for_existing_template_image
          @template_image_path = options[:template_image_path] || DEFAULT_IMAGE_PATH
          puts @template_image_path
          return unless File.exist?(@template_image_path)
        ...
        def generate_new_favicon_using_favico_letter
          return if File.exist?(@template_image_path)
          ...
          favicon_color = options[:favicon_color] || DEFAULT_PRIMARY_COLOR
